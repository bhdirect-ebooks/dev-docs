...
LESSON<em>maker</em> markup is <em>**intended for responsive devotionals or study guides**</em> that specifically focus on passages or books of Scripture.
...
Used for marking workbook question text and answer locations wherever it is desirable to give the user a text box in which to record their answer to a specific question in the text.

Question data attribute to be used on the element surrounding the question text.

`data-wb-question="{id}"`

Optionally the workbook question may have an associated answer data attribute to be used on the element in which the answer text will be placed. The reading application will replace the indicated element with the answer UI and ultimately the answer text.

`data-wb-answer="{id}"`

The id values for question and answer pairs must match each other, and be unique within the EPUB (no other question-answer pair uses the same id value).

If there is no answer element indicated the application should assume that the answer will follow the question immediately.
Example usage:

```html
<ol>
     <li data-wb-question="1">Barnabas and Paul were dedicated Christian leaders who had been chosen by the Holy Spirit to spread the gospel to the Gentile world. How did the people respond to Paul’s message (Acts 13:42–45, 50)?</li>
     <li data-wb-question="B">Sometimes Christians become discouraged when others don’t respond positively to their witness for Christ. How do you normally respond when someone rejects your attempts to witness to them about Jesus?</li>
</ol>
<ol>
     <li data-wb-answer="1"></li>
     <li data-wb-answer="B"></li>
</ol>
```
